...
fetch("http://localhost:3000/prendas")
  .then(res => res.json())
  .then(data => {
    productos = data.map(prenda => ({
      id: prenda.ID_Prenda,
      title: prenda.Nombre,
      description: prenda.Descripcion,
      price: parseFloat(prenda.precioBase),
      image: prenda.opcionesColor || "placeholder.jpg",    }));

    if (contenedorProductos) renderProductos();
    if (contenedorFavoritos) renderFavoritos();
  })
  .catch(err => console.error("Error cargando prendas desde backend:", err));
